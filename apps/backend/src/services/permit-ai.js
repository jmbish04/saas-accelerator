...
exports.PermitAIService = void 0;
...
class PermitAIService {
    ...
    async analyzePermitData(permits, question) {
        try {
    ...
    prepareDataSummary(permits) {
        if (!permits || permits.length === 0) {
            return 'No permit data available.';
        }
        // Basic statistics
        const totalPermits = permits.length;
        const permitTypes = permits.reduce((acc, permit) => {
            const type = permit.permitType || 'Unknown';
            acc[type] = (acc[type] || 0) + 1;
            return acc;
        }, {});
        const statuses = permits.reduce((acc, permit) => {
...
${permits.slice(0, 3).map(p => `- ID: ${p.id}, Type: ${p.permitType || 'N/A'}, Status: ${p.currentStatus || 'N/A'}, Address: ${p.streetNumber || ''} ${p.streetName || 'N/A'}, Description: ${(p.description || '').substring(0, 100)}...`).join('\n')}
    `.trim();
    }
}
exports.PermitAIService = PermitAIService;
